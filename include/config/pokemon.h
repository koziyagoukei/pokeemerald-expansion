...
#define P_UPDATED_TYPES             GEN_LATEST  // Some Pokemon have received type updates after their introduction.
#define P_UPDATED_STATS             GEN_LATEST  // Some Pokemon have received base stat updates after their introduction.
#define P_UPDATED_ABILITIES         GEN_LATEST  // Since Gen 4, certain Pokemon have had their abilities changed.
#define P_UPDATED_EGG_GROUPS        GEN_LATEST  // Since Gen 8, certain Pokemon have gained new egg groups.
#define P_UPDATED_FRIENDSHIP        GEN_LATEST  // Since Gen 8, the base friendship of certain Pokemon was changed.
#define P_UPDATED_EVS               GEN_LATEST  // Since Gen 4, some Pokemon have received EV updates.
#define P_UPDATED_EXP_YIELDS        GEN_LATEST  // Since Gen 5, some Pokemon have received base Experience changes.
...
#define P_LVL_UP_LEARNSETS          GEN_LATEST  // Choose which set of Level Up Learnsets to use.
                                                ...
                                                // GEN_8: Use the following priority: BDSP for Gen1-4 Pokemon, then LA for species introduced in that game, then SwSh for species present in those games. Otherwise, use GEN_7.
                                                // GEN_9: SV For species present in those games. Otherwise use GEN_8.
#define P_EVOLUTION_LEVEL_1_LEARN   GEN_LATEST  // Since Gen 8, Pokemon evolving while at level 1 do not get the chance to learn their level 1 moves, while before they did.

// Evolution settings
#define P_FRIENDSHIP_EVO_THRESHOLD   GEN_LATEST  // Since Gen 8, Pokemon that evolve by friendship evolve at or above 160 friendship instead of 220.
...
#define P_INCENSE_BREEDING             GEN_LATEST                // Since Gen 9, cross-generation Baby Pokemon don't require Incense being held by the parents to be obtained via breeding.
#define P_EGG_HATCH_LEVEL              GEN_LATEST                // Since Gen 4, Pokemon will hatch from Eggs at level 1 instead of 5.
#define P_BALL_INHERITING              GEN_LATEST                // Since Gen 6, Eggs from the Day Care will inherit the Poke Ball from their mother. From Gen 7 onwards, the father can pass it down as well, as long as it's of the same species as the mother.
...
#define P_ABILITY_INHERITANCE          GEN_LATEST                // In B2W2, a female Pokemon has an 80% chance of passing down their Ability if bred with a male. Since Gen 6, the chance is 80% for regular Ability and 60% for Hidden Ability, and anyone can pass down their Abilities if bred with Ditto. NOTE: BW's effect: 60% chance to pass down HA and random for regular Ability has been omitted.
#define P_EGG_MOVE_TRANSFER            GEN_LATEST                // Starting in Gen 8, if two Pokemon of the same species are together in the Day Care, one knows an Egg Move, and the other has an empty slot, the other Pokemon will receive the Egg Move in the empty slot. In Gen 9, if a Pokemon holds a Mirror Herb, it will receive Egg Moves from the other regardless of species.
...
#define P_SHEDINJA_BALL             GEN_LATEST  // Since Gen 4, Shedinja requires a Poke Ball for its evolution. In Gen 3, Shedinja inherits Nincada's Ball.
...
#define P_GENDER_DIFFERENCES            TRUE        // If TRUE, Pokemon will have graphical-only differences when they're female. Female *forms* such as Meowstic are not affected by this.
#define P_CUSTOM_GENDER_DIFF_ICONS      TRUE        // If TRUE, more Pokemon will have custom icons for their female differences, such as Hippopotas and Hippowdon.
#define P_ARCEUS_UNIQUE_FORM_ICONS      GEN_LATEST  // Since Gen 9, Arceus changes its icon to reflect its current form.
#define P_FOOTPRINTS                    TRUE        // If TRUE, Pokemon will have footprints (as was the case up to Gen 5 and in BDSP). Disabling this saves some ROM space.
#define P_TWO_FRAME_FRONT_SPRITES       TRUE        // In Pokemon Emerald, Pokemon front sprites always consist of two frames. This config can revert it to only use the first frame, as is the case in the other Gen 3 games.
#define P_GBA_STYLE_SPECIES_GFX         FALSE       // By default, Pokemon sprites use Gen 4/5's style. This config can revert them back to GBA-style. NOTE: B_ENEMY_MON_SHADOW_STYLE is disabled when using these sprites for now.
#define P_GBA_STYLE_SPECIES_ICONS       FALSE       // By default, Pokemon have their updated icons. This config can revert them back to GBA-style.
#define P_GBA_STYLE_SPECIES_FOOTPRINTS  FALSE       // By default, Pokemon have their updated footprints. This config can revert them back to GBA-style.
