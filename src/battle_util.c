...
static inline void MulByTypeEffectiveness(struct DamageContext *ctx, uq4_12_t *modifier, enum Type defType)
{
    ...
    // B_WEATHER_STRONG_WINDS weakens Super Effective moves against Flying-type Pokemon
...
bool32 CanTargetBattler(enum BattlerId battlerAtk, enum BattlerId battlerDef, enum Move move)
{
    ...
    &&  gBattleMons[battlerAtk].volatiles.healBlock)
        return FALSE;   // Pokemon affected by Heal Block cannot target allies with Pollen Puff
